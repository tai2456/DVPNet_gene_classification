...
class SPNJointGMMAttrVec(nn.Module):
    ...
    def _standardize_per_sample(self, x_bkqd: torch.Tensor,
                                mask_bk: Optional[torch.Tensor] = None) -> torch.Tensor:
       
        B, K, q, d = x_bkqd.shape
        x = x_bkqd.reshape(B, K, q * d)  # (B,K,qd)

        if mask_bk is None:
            with torch.no_grad():
                mu  = x.mean(dim=1, keepdim=True)                         # (B,1,qd)
                std = x.std(dim=1, keepdim=True).clamp_min(1e-3)          # (B,1,qd)
            xz = (x - mu) / std
            return xz.reshape(B * K, q * d)

        # mask あり（padding等を除外して平均/分散）
        m = mask_bk.reshape(B, K, 1).to(dtype=x.dtype)                    # (B,K,1)
        with torch.no_grad():
            den = m.sum(dim=1, keepdim=True).clamp_min(1.0)              # (B,1,1)

            mu = (x * m).sum(dim=1, keepdim=True) / den                  # (B,1,qd)
            var = ((x - mu) ** 2 * m).sum(dim=1, keepdim=True) / den     # (B,1,qd)
            std = var.sqrt().clamp_min(1e-3)                             # (B,1,qd)
    ...
    def per_patch_loglik(self, x_bkqd: torch.Tensor) -> torch.Tensor:
        B, K, q, d = x_bkqd.shape
        x_flat = self._standardize_per_sample(x_bkqd, mask_bk=None)       # (B*K, q*d)
        ll = self.patch_pc.forward_ll(x_flat)                             # (B*K,)
        ll = torch.nan_to_num(ll, neginf=-1e9, posinf=1e9)
    ...
    def per_patch_loglik_masked(self, x_bkqd: torch.Tensor, mask_bk: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        # まず「サンプルごと・mask考慮」で標準化（B境界を保持したまま）
        x_flat = self._standardize_per_sample(x_bkqd, mask_bk=mask_bk)    # (B*K, q*d)

        mask = (mask_bk.reshape(B * K) > 0.5)
        ll_full = torch.full((B * K,), fill_value=-100.0,
                             device=x_bkqd.device, dtype=x_bkqd.dtype)

        if mask.any():
            llv = self.patch_pc.forward_ll(x_flat[mask])
            llv = torch.nan_to_num(llv, neginf=-1e9, posinf=1e9)
            ll_full[mask] = llv
...
class _ClassConditionalGroup(nn.Module):
    ...
    def mean_loglik_masked(self, x_bkqd: torch.Tensor, mask_bk: Optional[torch.Tensor]) -> torch.Tensor:
        outs=[]
        for c in range(self.C):
...
class SPNMultiAttrGroupsMC(nn.Module):
    ...
    def joint_logp_per_class_masked(self, feats: Dict[str,torch.Tensor], masks: Dict[str,Optional[torch.Tensor]]) -> torch.Tensor:
        ...
        for g in self.group_names:
            ...
            x = feats[g]
            m = masks.get(g, None)
            l = group.mean_loglik_masked(x, m)     # (B,C)
            logs.append(torch.nan_to_num(l, neginf=-1e9, posinf=1e9))
        joint = torch.stack(logs, 0).sum(0) + self._log_prior().view(1,self.C)
        with torch.no_grad():
            print("joint min/max:", joint.min().item(), joint.max().item())
            diff = joint[:,0] - joint[:,1]
            print("joint diff mean/max:", diff.mean().item(), diff.abs().max().item())
    ...
    def posterior_logprobs(self, feats, masks=None):
        masks = masks or {g: None for g in self.group_names}

        # 1) joint = SUM(log p(x|c)) + log prior
        joint = self.joint_logp_per_class_masked(feats, masks)

        # 2) 数値異常だけ処理
        joint = torch.nan_to_num(joint, neginf=-1e30, posinf=1e30)

        # 3) 論文通りのベイズ正規化
        return torch.log_softmax(joint, dim=1)
...
class NPC_VECTORS_MC(nn.Module):
    ...
    def loss_and_stats_from_feats(
        self,
        x_bkqd: torch.Tensor,
        mask_bk: Optional[torch.Tensor],
        y: torch.Tensor,
        class_weights: Optional[torch.Tensor] = None,
        entropy_weight: float = 1,          # Shannon (H) 係数
    ):
        device = next(self.parameters()).device
        B = x_bkqd.size(0)
        y = y.long().to(device)

        # logp = log P(class | x)  (B,C)
        logp = self.log_posteriors_from_feats(x_bkqd, mask_bk)
        ...
        # --- Shannon entropy H(P) ---
        prob = logp.exp()
        entropy = -(prob * logp).sum(dim=1).mean()  # H >= 0
        # total loss
        loss = ce - float(entropy_weight) * entropy 
        print("ce",ce)
        print("entropy",float(entropy_weight) * entropy)
